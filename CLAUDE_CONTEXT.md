...
- **2026-07-08 (sync-ownership refactor — in progress):** Stepped back from reactive bug-fixing to plan the FareHarbor→app sync as a whole. Audited the three writers (iCal 90s poll, webhook on booking events, v2 hourly scraper) against the real code and found the root disease behind most recent bugs: **iCal and v2 are two near-duplicate upserters to `tour_availabilities` with no per-field ownership**, so they overwrite each other every cycle (last writer wins; iCal wins ~39/40 by frequency, so the fast-but-fuzzy source usually beats the slow-but-accurate one). Governing principle agreed with Fede: **one owner per field**, enforced by making each writer touch a disjoint set of fields. Plan is three deploy-and-eyeball steps: (1) **bikes — DONE this session** (per-key merge, see below / §2); (2) guide — v2 authoritative, iCal a *cautious* gap-fill that fills the guide immediately only when the LOCATION label is unambiguous, blank otherwise, and never overwrites a value v2 has set; (3) freeze finished tours (snapshot-on-complete; Fede confirmed he wants past tours immutable) + collapse the duplicated duration-buffer logic (iCal's `computeBufferedMinutes` vs v2's inline `buffer=isFoodTour?60:30`, currently identical numbers in two places — the exact shape that let the F3 duration drift) into one shared function. **Step 1 shipped:** `bikes_needed` now merges per-key via atomic `json_set` in both upserts (iCal owns A/E/B/AC/AT, v2 owns GT), `total_bikes` recomputed as the combined sum — verified stable across repeated iCal/v2 cycles on a mixed tour, ending the oscillation; self-heals within one v2 cycle, no migration. Also noticed while auditing: `cancel-booking.js` now exists (older doc note said it didn't), and `routes/guides.js` + `routes/reviews.js` are not yet in the §3 directory map.
