...
| Item | Status |
|---|---|
| Booking `created_at` overwrite bug | **Fixed in code, migration pending confirmation.** The 90-second iCal sync was unconditionally overwriting `tour_availabilities.bookings_json` with a freshly-parsed version lacking `created_at` (iCal text has no per-booking timestamp at all) — silently wiping out the value the webhook correctly sets within ~90 seconds of every booking. This defaulted the "Can keep bikes after tour" flag (and any other "booked before X" logic) to showing on every booking, not just old ones. `ical.js` now merges forward any `created_at` already present before overwriting. `scripts/fixes/backfill-booking-created-at.js` backfills currently-missing values from `action_log`'s `booking_received` history into both `tour_availabilities.bookings_json` and the `bookings` ledger |
...
| F3/F3P duration fix (`scripts/fixes/fix-past-f3-duration.js`) | **In progress, not yet confirmed fixed in the UI.** Forces every F3/F3P `guide_tour_hours` row to 270 minutes (4h30m), after the earlier date-scoped version of the same fix failed to catch at least one row. Root cause of the date-comparison miss not fully confirmed — the unconditional version was shipped as a more robust workaround rather than a full root-cause fix |
| Guide review-rate / hours re-verification | Pam's (and others') data for the current billing cycle (23 Jun–22 Jul) needed multiple manual corrections this session (deleted-then-restored manual seed rows, missing `start_at` on restore, stale frozen durations on completed tours). Worth a full re-verification pass across all guides, not just Pam, once the F3 duration fix above is confirmed |
| `guide_mismatch` / `bike_data_anomaly` alert types | Live in code but not yet observed firing on real data — unverified in practice |
| Completed/past tour re-sync | Completed/past tours do not get re-synced once they age out of the live FareHarbor feed. Any future policy change to a duration/field will hit the same frozen-past-data problem and will need an explicit migration script under `scripts/fixes/`, not just a code fix |
