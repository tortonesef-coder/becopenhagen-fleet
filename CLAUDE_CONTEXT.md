...
- **2026-07-15 (fix password-reset link domain — was pointing at old host):** The app's canonical domain is **app.becopenhagen.dk** (per Fede). But both password reset / account-setup links in `auth.js` (the self-serve `/forgot-password-email` and the admin-triggered one) hardcoded `https://fleet.interestingtours.dk/reset-password?token=...` — so new-hire invite links (e.g. Dimitra's) pointed at the wrong host. Replaced both with a single `APP_BASE_URL` constant (default `https://app.becopenhagen.dk`, overridable via env, trailing slash stripped), so the two send paths can't drift again. Note for future: the reset-password base URL is now app.becopenhagen.dk, not fleet.interestingtours.dk.

- **2026-07-15 (cleanup: quiet expected week-6 400s; remove one-off diagnostics):** Two loose ends. (1) The scraper logged `week 5/6 attempt: HTTP 400` on every run and retried them — but a month spans at most 6 week-rows and most use 4-5, so requesting a week-row past month-end returns 400 = "no such week", which is EXPECTED, not a failure. `fetchMonth` now treats a 400 as past-end: stop the week loop, don't retry, don't log it. Only non-400 failures produce noise now, so a real error will actually stand out. (Safe: weeks are sequential, so if week N is past-end so is N+1; and if week 1 itself 400'd, `results.length===0` still throws.) (2) Removed the three one-off investigation diagnostics (diag-aug31, diag-jul19, diag-sync-freshness). Kept the reusable ones: `diag-resources.js` (re-validate resource bike counts), `diag-today-bikes.js` (recompute the board's peak by hand), `diag-login-readiness.js` (check all guides have passwords set).

- **2026-07-15 (H3 tours silently deleted — item ID missing from fetch list):** The scraper logged `✗ Removing cancelled slot: 2026-08-03 11:30 H3` and deleted a **real booked H3** (2 pax, Martin van Herwijnen, paid — confirmed in FareHarbor). Root cause: H3 (item 741878) is in `TOUR_ITEMS` (so the code recognizes it) but was **never added to `ALL_ITEM_IDS`**, the item-id string in the actual fetch URL. So the scraper never fetched ANY H3, never saw them in the feed, and the cancellation sweep deleted every H3 DB row as "gone" — booked or not, every run. Note: no H3 line appears anywhere in the sync log, which is the tell. Fixes: (1) added 741878 to `ALL_ITEM_IDS`. (2) **Hardened the sweep so this can't recur:** it now only cancel-checks feed types actually FETCHED this run (`fetchedFeeds` from `all`); rows of a feed type absent from the fetch (missing item ID, or that item errored) are skipped with a warning, never deleted. The deleted H3 rows repopulate on the next scrape now that H3 is fetched.
