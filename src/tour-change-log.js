...
function logTourChange(db, { availability_id, feed_id, start_date, field, old_value, new_value, source, raw_data }) {
  // Skip no-op logs (both null/undefined, or identical values)
  if ((old_value ?? null) === (new_value ?? null)) return;
  try {
    db.prepare(`
      INSERT INTO tour_change_log (availability_id, feed_id, start_date, field, old_value, new_value, source, raw_data)
      VALUES (?,?,?,?,?,?,?,?)
    `).run(availability_id, feed_id || null, start_date || null, field, old_value ?? null, new_value ?? null, source || null, raw_data || null);
  } catch (e) {
